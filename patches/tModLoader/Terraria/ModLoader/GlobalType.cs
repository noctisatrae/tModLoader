...
{
	/// <summary>
	/// Index in the <see cref="GlobalList{TGlobal}.Globals"/> array.
	/// </summary>
	public int StaticIndex;

	/// <summary>
	/// Index in the <see cref="IEntityWithGlobals{TGlobal}.EntityGlobals"/> or -1 if the global does not have a <see cref="GlobalType{TGlobal}.SlotPerEntity"/>
	/// </summary>
	public int PerEntityIndex;
}

public abstract class GlobalType<TGlobal> : ModType where TGlobal : GlobalType<TGlobal>
{
	public GlobalIndex Index { get; internal set; }
	...
	/// If true, the global will be assigned a <see cref="GlobalIndex.PerEntityIndex"/> at load time, which can be used to access the instance in the <see cref="IEntityWithGlobals{TGlobal}.EntityGlobals"/> array. <br/>
	...
	protected override void Register()
	{
		...
		Index = GlobalList<TGlobal>.Register((TGlobal)this);
	}
	...
	{
		var index = baseInstance.Index;
		if (index.PerEntityIndex >= 0) {
			result = (TResult)entityGlobals[index.PerEntityIndex];
			return result != null;
		}
		result = (TResult)GlobalList<TGlobal>.Globals[index.StaticIndex];
		return true;
	}
